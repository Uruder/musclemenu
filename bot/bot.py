...
def get_main_menu(language="en"):
    return types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="Daily Plan" if language == "en" else "Дневной рацион" if language == "ru" else "Денний раціон", callback_data="daily_plan")],
        [types.InlineKeyboardButton(text="Language" if language == "en" else "Язык" if language == "ru" else "Мова", callback_data="switch_language")]
    ])

# Быстрое меню
def get_quick_menu(language="en"):
    return types.ReplyKeyboardMarkup(
        keyboard=[
            ...
            [types.KeyboardButton(text="Goal" if language == "en" else "Цель" if language == "ru" else "Мета"),
             types.KeyboardButton(text="Share" if language == "en" else "Поделиться" if language == "ru" else "Поділитися")],
...
def get_back_menu(text_to_share="", language="en"):
    return types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="Back" if language == "en" else "Назад", callback_data="back_to_main")]
    ])
...
async def generate_daily_recipe(user_data):
    ...
    language = user_data.get("language", "en")  # Получаем язык пользователя
    ...
    activity_multipliers = {
        ...
        "низкая": 1.2, "средняя": 1.55, "высокая": 1.9,
        "низький": 1.2, "середній": 1.55, "високий": 1.9
    }
    ...
    if preferences:
        available_recipes = [recipe for recipe in RECIPES if any(pref in recipe["recipe"]["en"].lower() for pref in preferences)]
    ...
    ration_text = f"Daily Plan if language == 'en' else 'Дневной рацион' if language == 'ru' else 'Денний раціон'\n\n{goal_text[language]}\n\n"
    for i, recipe in enumerate(selected_recipes):
        ration_text += f"- {meal_names[language][i]}: {recipe['name'][language]} ({recipe['calories']} kcal, protein: {recipe['protein']}g, fat: {recipe['fat']}g, carbs: {recipe['carbs']}g)\n  {recipe['recipe'][language]}\n\n"
    ration_text += f"Total calories: {total_cal:.0f} kcal (approximately {total_calories:.0f} kcal needed for {goal.replace('_', ' ').title()})."
...
async def start(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if user:
            ...
            markup = types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="Stars (50 XTR)", callback_data="pay_stars"),
                 types.InlineKeyboardButton(text="Card (500 UAH)", callback_data="pay_stripe")],
                [types.InlineKeyboardButton(text="Back", callback_data="back_to_main")]
            ])
            await message.reply(
                TEXTS[language]["daily_plan"] if language in TEXTS and "daily_plan" in TEXTS[language] else "Subscription for 30 days access to daily meal plan:",
                reply_markup=markup
            )
        ...
        await message.reply("💪 *Hello! I’m MuscleMenu!* I’ll help you gain mass.\nLet’s get you registered. What’s your name?", reply_markup=get_quick_menu("en"), parse_mode="Markdown")
        ...
        await message.reply("An error occurred. Please try again later.", reply_markup=get_quick_menu("en"), parse_mode="Markdown")
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("What’s your height (in cm)?", reply_markup=get_quick_menu("en"))
        ...
        await message.reply("An error occurred. Please try again later.", reply_markup=get_quick_menu("en"))
...
async def process_height(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("What’s your weight (in kg)?", reply_markup=get_quick_menu("en"))
        await state.set_state(UserForm.weight)
        logging.info(f"Processed height and set weight state for user {message.from_user.id}")
    except ValueError:
        await message.reply("Please enter a number. What’s your height (in cm)?", reply_markup=get_quick_menu("en"))
    except Exception as e:
...
async def process_weight(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("How old are you?", reply_markup=get_quick_menu("en"))
        await state.set_state(UserForm.age)
        logging.info(f"Processed weight and set age state for user {message.from_user.id}")
    except ValueError:
        await message.reply("Please enter a number. What’s your weight (in kg)?", reply_markup=get_quick_menu("en"))
    except Exception as e:
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("What’s your activity level (low/medium/high)?", reply_markup=get_quick_menu("en"))
        await state.set_state(UserForm.activity)
        logging.info(f"Processed age and set activity state for user {message.from_user.id}")
    except ValueError:
        await message.reply("Please enter a number. How old are you?", reply_markup=get_quick_menu("en"))
    except Exception as e:
...
async def process_activity(message: types.Message, state: FSMContext):
    ...
    if activity_level not in ["low", "medium", "high", "низкая", "средняя", "высокая", "низький", "середній", "високий"]:
        await message.reply("Please specify: low/medium/high (or низкая/средняя/высокая, низький/середній/високий). What’s your activity level?", reply_markup=get_quick_menu("en"))
        return
    try:
        await state.update_data(activity_level=activity_level)
        await message.reply("How many workouts do you have per week?", reply_markup=get_quick_menu("en"))
...
async def process_workouts(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("What’s your goal? (weight gain/weight loss/weight maintenance)\nSpecify in Russian, English, or Ukrainian (e.g., weight gain, снижение веса, набір маси).", reply_markup=get_quick_menu("en"))
        ...
        await message.reply("Please enter a number. How many workouts do you have per week?", reply_markup=get_quick_menu("en"))
    except Exception as e:
...
async def process_goal(message: types.Message, state: FSMContext):
    ...
    if goal not in valid_goals:
        await message.reply("Please specify a valid goal: weight gain/weight loss/weight maintenance (or аналог на русском/украинском).", reply_markup=get_quick_menu("en"))
        return
    try:
        await state.update_data(goal=valid_goals[goal])
        await message.reply("What foods do you like? (E.g., 🍗 chicken, 🥚 eggs, 🍚 rice)\nLeave empty if no preferences.", reply_markup=get_quick_menu("en"))
...
async def process_preferences(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await db.add_user(
            ...
            data["age"], data["activity_level"], data["workouts_per_week"], preferences, "en", data.get("goal", "gain_mass")
        )
        await message.reply("✅ *Data saved!* What’s next?", reply_markup=get_quick_menu("en"), parse_mode="Markdown")
        await state.clear()
        ...
        await message.reply("An error occurred while saving data. Please try again later.", reply_markup=get_quick_menu("en"))
...
async def daily_plan(callback: types.CallbackQuery):
    ...
    try:
        ...
        if not user:
            await callback.message.reply("Register first!", reply_markup=get_main_menu("en"))
            return
        language = user["language"]
        ...
        await callback.message.reply("An error occurred. Please try again later.", reply_markup=get_main_menu("en"))
...
async def back_to_main(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.reply("An error occurred. Please try again later.", reply_markup=get_quick_menu("en"))
...
async def pay_stars(callback: types.CallbackQuery):
    ...
    try:
        ...
        await bot.send_invoice(
            ...
            title="Subscription for 30 days",
            description="Access to daily meal plan for 30 days",
            provider_token="",
            currency="XTR",
            prices=[types.LabeledPrice(label="Subscription", amount=50)],
            payload="subscription_stars"
        )
    except Exception as e:
        logging.error(f"Error in pay_stars for user {callback.from_user.id}: {e}")
        await callback.message.reply("An error occurred while processing payment. Please try again later.", reply_markup=get_main_menu(language))
...
async def pay_stripe(callback: types.CallbackQuery):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="Pay", url=payment_url)]
        ])
        await callback.message.reply(
            "Go to the link to pay (500 UAH):",
            reply_markup=markup
        )
    except Exception as e:
...
async def successful_payment(message: types.Message):
    ...
    try:
        ...
        await message.reply("An error occurred while processing payment. Please try again later.", reply_markup=get_main_menu(language))
...
async def quick_daily_plan(message: types.Message):
    ...
    user = await db.get_user(message.from_user.id)
    if not user:
        await message.reply("Register first!", reply_markup=get_quick_menu("en"))
        return
    language = user["language"]
...
@dp.message(lambda message: message.text in ["Goal", "Цель", "Мета"])
async def quick_goal(message: types.Message):
    logging.info(f"Received quick menu 'Goal' from user {message.from_user.id} with text: {message.text}")
    user = await db.get_user(message.from_user.id)
    if not user:
        await message.reply("Register first!", reply_markup=get_quick_menu("en"))
        return
    ...
    goal = user["goal"]
    preferences = user.get("preferences", "").lower().split(", ") if user.get("preferences") else []
    ...
    if goal == "gain_mass":
        total_calories *= 1.15  # Набор массы — +15%
        weekly_weight_gain = 0.5  # Гипотетический прирост веса (0.5 кг в неделю при избытке калорий)
    elif goal == "lose_weight":
        total_calories *= 0.85  # Снижение веса — -15%
        weekly_weight_gain = -0.5  # Гипотетическое снижение веса (0.5 кг в неделю при дефиците калорий)
    elif goal == "maintain_weight":
        total_calories *= 1.0  # Поддержание веса — без изменений
        weekly_weight_gain = 0.0  # Нет изменения веса

    # Шкала прогресса (простая реализация, обновляемая раз в неделю)
    now = datetime.now()
    last_update = user.get("last_weight_update", datetime.now() - timedelta(days=7))  # Предполагаем, что обновление было неделю назад
    progress = "No progress data yet"  # Изначально пустая шкала
    if (now - last_update).days >= 7:  # Обновляем прогресс раз в неделю
        # Здесь можно добавить логику получения нового веса пользователя (например, через запрос или ввод)
        new_weight = weight + weekly_weight_gain  # Гипотетический новый вес
        progress_percentage = min(100, max(0, (new_weight - weight) / weekly_weight_gain * 100 if weekly_weight_gain != 0 else 0))
        progress = f"Progress: {progress_percentage:.1f}% towards {goal.replace('_', ' ').title()} (Current weight: {new_weight:.1f} kg)"
        # Обновляем данные в базе
        await db.update_user_weight_update(message.from_user.id, now, new_weight)

    # Формируем текст ответа
    goal_text = {
        "en": f"Daily Calorie Goal: {total_calories:.0f} kcal\nWeekly Weight Change: {weekly_weight_gain:+.1f} kg\n{progress}",
        "ru": f"Норма калорий в день: {total_calories:.0f} ккал\nИзменение веса за неделю: {weekly_weight_gain:+.1f} кг\n{progress}",
        "uk": f"Добова норма калорій: {total_calories:.0f} ккал\nЗміна ваги за тиждень: {weekly_weight_gain:+.1f} кг\n{progress}"
    }
    await message.reply(goal_text[language], reply_markup=get_quick_menu(language))

@dp.message(lambda message: message.text in ["Share", "Поделиться", "Поділитися"])
async def quick_share(message: types.Message):
    logging.info(f"Received quick menu 'Share' from user {message.from_user.id} with text: {message.text}")
    user = await db.get_user(message.from_user.id)
    if not user:
        await message.reply("Register first!", reply_markup=get_quick_menu("en"))
        return
    language = user["language"]
    # Получаем последний рацион для sharing
    ration = await generate_daily_recipe(user)
    share_url = f"https://t.me/share/url?url={ration.replace(' ', '%20')}"
    await message.reply("Share your meal plan:", reply_markup=get_quick_menu(language))
...
dp.message.register(quick_goal)
dp.message.register(quick_share)
